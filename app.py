...
JSON_DATA_PATH = "./final_merged_all.json"
RAW_DATA = []
# 5. 히스토리 파일 경로 설정
...
def identify_discipline_from_filename(file_name):
    """
    파일명에 포함된 키워드를 분석하여 공종을 판별합니다.
    """
    # 공종별 매핑 키워드
    discipline_map = {
        "건축": ["건축", "ARCH", "A-", "평면도", "단면도"],
        "기계": ["기계", "MECH", "M-", "설비", "공조", "위생"],
        "전기": ["전기", "ELEC", "E-", "계통도", "전력", "조명"],
        "조경": ["조경", "LAND", "L-", "식재", "시설물"],
        "토목": ["토목", "CIVIL", "C-", "구조물", "토공", "배수"],
        "건설사업관리법규": ["법규", "지침", "시방서", "계약", "LEGAL"]
    }
    ...
    for discipline, keywords in discipline_map.items():
        if any(kw in file_name_upper for kw in keywords):
            return discipline
            
    return "일반" # 매칭되는 것이 없을 경우
            ...
            with st.status(f"📄 '{target_file.name}' 분석 중...", expanded=True) as status:
                ...
                try:
                    ...
                    for i in range(total_pages):
                        ...
                        # 1. 파일 이름에서 공종 판별 (target_file.name 활용)
                        discipline = identify_discipline_from_filename(target_file.name)
                        status.write(f"📂 파일명 기반 공종 판별: **{discipline}**")

                        # 2. 판별된 공종을 바탕으로 DB 검색 (RAG)
                        # 검색어에 공종을 결합하여 시방서와 법규를 정확히 찾아옵니다.
                        search_query = f"{discipline} 공사 표준시방서 시공 기술 기준 및 건설법규 지침"
                        retrieved_docs = retrieve_and_rerank(search_query, top_k=5)

                        # 3. 비전 AI 분석 실행
                        # 이때, "기계 분야 전문 기술 검토를 수행하라"는 식으로 공종을 명시합니다.
